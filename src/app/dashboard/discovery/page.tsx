            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="flex items-center gap-6">
                    <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center shadow-xl shadow-indigo-100 relative overflow-hidden">
                        <Target className="w-8 h-8 text-white z-10" />
                        <div className="absolute inset-0 bg-white/10 rotate-45 translate-y-8" />
                    </div>
                    <div>
                        <h1 className="text-4xl font-black tracking-tighter uppercase text-slate-900">Discover <span className="text-gradient">People</span></h1>
                        <p className="text-slate-400 font-mono text-[10px] uppercase tracking-[0.3em] font-bold">{connections.length} Top Recommendations</p>
                    </div>
                </div>
                <Button className="bg-slate-900 text-white hover:bg-slate-800 rounded-2xl px-8 h-14 font-black uppercase text-[10px] tracking-widest transition-all hover:scale-105 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <AnimatePresence>
                    {connections.map((connection, index) => (
                        ...
                        >
                            <Card className="glass-card rounded-[2.5rem] overflow-hidden group h-full flex flex-col border-white shadow-lg shadow-slate-200/40">
                                <div className="relative h-44 bg-gradient-to-br from-slate-50 to-indigo-50/30 flex items-center justify-center border-b border-white">
                                    <div className="w-24 h-24 rounded-full bg-white border border-slate-100 flex items-center justify-center relative shadow-sm group-hover:scale-110 transition-transform duration-500">
                                        <span className="text-3xl font-black text-slate-800">{connection.full_name ? connection.full_name[0] : "U"}</span>
                                        <div className="absolute inset-[-4px] rounded-full border border-indigo-200 animate-pulse opacity-50" />
                                    </div>
                                    <div className="absolute top-4 right-4">
                                        <Badge className="bg-white border-slate-100 text-indigo-600 font-black text-[9px] px-3 h-6 shadow-sm">
                                ...
                                <CardContent className="p-8 space-y-6 flex-1 flex flex-col">
                                    <div className="space-y-1">
                                        <h2 className="text-2xl font-black text-slate-800 tracking-tight leading-none uppercase">{connection.full_name || "Unknown User"}</h2>
                                        <p className="text-indigo-600 font-mono text-[9px] font-black uppercase tracking-[0.2em]">{connection.academic_aim || "University Student"}</p>
                                    </div>
                                    ...
                                    <AnimatePresence>
                                        {icebreakers[connection.id] && (
                                            <motion.div
                                                ...
                                                className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 relative"
                                            >
                                                <MessageCircle className="w-4 h-4 text-emerald-600 absolute top-3 right-3" />
                                                <p className="text-xs text-emerald-700 leading-relaxed font-black mb-1 text-[9px] uppercase tracking-widest">Icebreaker:</p>
                                                <p className="text-xs text-emerald-800 leading-relaxed font-bold italic">
                                                    {icebreakers[connection.id]}
                                                </p>
                                            </motion.div>
                                        )}
                                    ...
                                    <div className="pt-4 mt-auto flex gap-3">
                                        <Button
                                            size="icon"
                                            variant="outline"
                                            className="flex-1 rounded-xl border-slate-100 hover:bg-slate-50 transition-all h-12"
                                            onClick={() => handleGetIcebreaker(connection.id)}
                                            disabled={icebreakerLoading[connection.id]}
                                        >
                                            {icebreakerLoading[connection.id] ? <Loader2 className="w-5 h-5 animate-spin text-indigo-600" /> : <MessageCircle className="w-5 h-5 text-indigo-600" />}
                                        </Button>
                                        <Button className="flex-[2] rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[10px] tracking-widest shadow-lg shadow-indigo-100 h-12">
