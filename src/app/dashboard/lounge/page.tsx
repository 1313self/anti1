            ...
            <main className="max-w-4xl mx-auto relative z-10 space-y-12">
                <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 py-8">
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-6">
                            <div className="w-16 h-16 rounded-2xl bg-emerald-600 flex items-center justify-center shadow-xl shadow-emerald-100 relative overflow-hidden">
                                <MessageSquare className="w-8 h-8 text-white z-10" />
                                <div className="absolute inset-0 bg-white/10 rotate-45 -translate-y-8" />
                            </div>
                            <div>
                                <h1 className="text-4xl font-black tracking-tighter uppercase text-slate-900">The <span className="text-gradient">Lounge</span></h1>
                                <p className="text-slate-400 font-mono text-[10px] uppercase tracking-[0.3em] font-bold">Campus Pulse Feed</p>
                            </div>
                        </div>
                    </div>
                </header>

                <Card className="glass-card rounded-[2.5rem] border-white shadow-xl shadow-slate-200/50 overflow-hidden">
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1 relative">
                                <textarea
                                    ...
                                    className="w-full h-24 p-4 rounded-2xl bg-slate-50/50 border border-slate-100 text-slate-700 font-medium outline-none focus:border-indigo-400 focus:bg-white transition-all resize-none"
                                />
                                <div className="absolute bottom-3 right-3 flex gap-2">
                                    <Button size="icon" variant="ghost" className="w-8 h-8 rounded-lg text-slate-400">
                                        <Hash className="w-4 h-4" />
                                    </Button>
                                    <Button size="icon" className="w-8 h-8 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-90">
                ...
                <div className="space-y-8">
                    <div className="flex items-center gap-4 px-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Recent Transmissions</span>
                    </div>
                    ...
                    <div className="space-y-6">
                        <AnimatePresence>
                            {SAMPLE_POSTS.map((post, index) => (
                                ...
                                >
                                    <Card className="glass-card rounded-[2.5rem] border-white shadow-lg shadow-slate-200/30 overflow-hidden group">
                                        <CardContent className="p-8 flex gap-6">
                                            <div className="w-14 h-14 rounded-2xl bg-white border border-slate-100 flex items-center justify-center shadow-sm shrink-0">
                                                <span className="text-xl font-black text-indigo-600">{post.user[0]}</span>
                                            </div>
                                            <div className="flex-1 space-y-4">
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <h3 className="text-lg font-black text-slate-900 tracking-tight uppercase leading-none">{post.user}</h3>
                                                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">{post.time} • #{post.tag}</span>
                                                    </div>
                                                    <Button variant="ghost" size="icon" className="w-8 h-8 rounded-lg text-slate-300">
                                                ...
                                                <div className="flex items-center gap-6 pt-2">
                                                    ...
                                                    <button className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-500 transition-colors ml-auto">
